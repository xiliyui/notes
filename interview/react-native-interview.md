...
8. 如何处理用户输入（例如点击事件）并作出相应的反应？
可以通过在组件中定义事件处理函数，并将其绑定到相应的用户输入事件（如按钮点击）上来处理用户输入。事件处理函数可以在用户交互发生时触发相应的操作。
...
10. 请描述一下 React Native 中的生命周期方法以及它们的用途。
React Native 中的生命周期方法包括 componentDidMount、componentDidUpdate、componentWillUnmount 等。这些方法可以在特定的生命周期阶段执行特定的操作，例如初始化数据、更新状态或清理资源。
